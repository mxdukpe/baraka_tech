...
//       // Vérifiez que data est bien un tableau avant de l'utiliser
...
useEffect(() => {
  const fetchAllProducts = async () => {
    console.log('Début du chargement des produits'); // DEBUG
    if (!token) {
      console.log('Token non disponible'); // DEBUG
      return;
    }
    ...
    try {
      ...
      console.log('Initialisation chargement'); // DEBUG

      while (hasMore) {
        try {
          console.log(`Chargement page ${currentPage}`); // DEBUG
          const response = await getProductsPage(token, currentPage);
          console.log('Réponse API:', response); // DEBUG
          
          if (!response.results || response.results.length === 0) {
            console.warn('Page vide reçue'); // DEBUG
            break;
          }
          ...
          console.log(`${products.length} produits chargés`); // DEBUG
        } catch (error) {
          console.error(`Erreur page ${currentPage}:`, error); // DEBUG
          hasMore = false;
          throw error;
        }
      }

      console.log('Total produits:', products.length); // DEBUG
...
const getPriceByCriterion = (product: Product) => {
  ...
  // Obtenir le prix en fonction du criterion
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: theme.text }]}>Nos Produits</Text>
      </View>
      ...
      <FlatList
        data={allProducts} // Utilisez allProducts au lieu de filteredProducts
        renderItem={renderProduct}
        keyExtractor={(item) => item.id.toString()}
        numColumns={2} // Ajoutez ceci pour afficher 2 colonnes
        columnWrapperStyle={styles.productRow} // Style pour l'alignement des colonnes
